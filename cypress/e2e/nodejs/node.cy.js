describe("Teste do cypress.config.js", () => {
  ...
  it("conta o total de arquivos da pasta API", () => {
    cy.task("lerPasta", "cypress/e2e/api").then((totalArquivos) => {
      cy.log(`Quantidade de arquivos: ${totalArquivos}`);
    });
  });

  it("conta o total de arquivos da pasta NODEJS", () => {
    cy.task("lerPasta", "cypress/e2e/nodejs").then((totalArquivos) => {
      cy.log(`Quantidade de arquivos: ${totalArquivos}`);
    });
  });
  ...
  it.skip("le variavel de ambiente do sistema operacional", () => {
    cy.task("lerEmail").then((retorno) => {
      cy.log(retorno);
    });
  });
});
